...
- Clause Engine (Stage 16, in STAGE_ORDER: **Pass 1** conditional decomposition — shart_particle, feil_shart, jawab_particle, jawab_shart; **Pass 2** candidate hal (`hal_clause`), tamyiz (`tamyiz_phrase`), mawsul+sila (`ism_mawsul`, `sila_mawsul`); `hal_detected` / `tamyiz_detected` / `sila_detected`; clause_analysis, SECTION 4g). Tightened so emphasis operators such as `إنَّ` (`ACC_TAWKID`) are not treated as conditional from connective hints alone; pipeline output now also exposes `transformation_result` for schema-compatible readers.
...
### E.1 Near-term syntax & Quran-gold focus (operational, March 2026)

These items refine **upstream classification and attachment** for the gold comparison harness; they do **not** replace the long-range stage priorities above.

**Master execution bookkeeping (Mar 2026):** Patches **1–9** are **classified** in **`PIPELINE_MASTER_MEMORY.md` Section 7.1** into **(A) engine-improvement**, **(B) comparator / tier hygiene** (incl. **Patch 9b** **`gold_parser_limit`** — **not** pure engine gain), and **(C) skipped** (**Patch 8**). **Patches 13–15** extend **(B)** (comparator-only): **`darf`/ظرف** bridge, **B32** fused **harf_jar** surfaces (**لنا** / **عليكم** / **عليهم** / **مما**), **particle**/**fael** vs **مضاف** (**2:43**). **Human-approved:** all three are **comparator normalization / bridges**, **not** engine-improvement — book gains under **(B)** at gates. **Dual baselines:** **§7.2** frozen **272/143** (post–**10–12**) and **§7.2.1** live **261/137** (post–**13–15**) are **both** valid; see **PIPELINE_MASTER_MEMORY.md** §7.2 note. **Checkpoint:** **§7.2.1** (**A/B/C**). **Patches 16–18 (Mar 24 2026):** implemented per **`PIPELINE_MASTER_MEMORY.md` §8** — **(16)** **(A)/(2)** Stage15, **(17)** **(1)** L17 **IDAFA** priority + **G007** gate, **(18)** **(3)** comparator **`case_bucket_mismatch`**; benchmarks **494 / 4000 / 10000** archived under **`data/quarantine_batch28/p16_18_exec/`** — **awaiting human approval** before the next execution batch. **Section 7.3:** gates **10** / **15** / **18** (done) / **pre-20** (split **(1)–(4)** gains).

**Batch 28.30 — closed (March 2026):** Reporting-verb **upstream** batch **validated** — **494**-row: **strict_structural_match 182→187**, **mismatch 287→283**, **pass_strict_ayahs 6**, **alignment_coverage 1.0**. **Next operational priority — L10B clause-span / locality-collapse:** diagnose and reduce cases where **L10B** `clause_units` (or downstream maps) still collapse long ayahs so **Stage 15** / **L17** matrix links pick **distant** dependents; builds on **28.24** `clause_locality` plumbing.

**Backlog — إذا وأخواتها (full family; explicit, separate from generic particles):**
**L17 B41** (Master Execution **Patch 11**) adds **narrow Quranic ظرف templates** for frequent surfaces (**إذا**/**إذ**, لما, كلما, قبل, مع, فوق, تحت, حول) — **not** a substitute for a dedicated **(1)** fused **operator / ظرف** decomposition batch or **(2)** **conditional vs temporal** disambiguation. **Decision:** keep **full** **إذا**-family work separate from **L10B** locality until scheduled; do **not** fold into generic particles or flat clause splitting.
...
| Date | Change | Notes |
|------|--------|------|
| 2026-03-25 | **L6 CV authority:** **`word2cv_authority`** = single helper for **`c1.cv_analysis`** and the reference script; **`G_WASL`** validation-only (**WARN**); authoritative CV isolated from C2a segment repairs | `src/word2cv_authority.py`, `fvafk/c1/cv_pattern.py`, `gate_wasl.py`, `gate_framework.py` |
| 2026-03-21 | **Patch 20 planning (diagnosis-first):** **B 4000** post–**19** — **`mismatch` 1378**, **`no_match` ~86%** of mismatch; top families **`mubtada`/`khabar`/`mafool_bih` ∧ «فعل»**, **`mafool_bih`∧«فاعل»**, **`mudaf_ilaih`∧«مفعول به»**; **A 494** / **C 10000** roles per **PIPELINE §7.6**; **Patch 19** = **infrastructure/alignment/integrity** only — not sold as **`mismatch`** drop (risk: none) | `PIPELINE_MASTER_MEMORY` §7.6 |
| 2026-03-24 | **Patch 20 (final — L14-only):** **`has_strong_true_verb_evidence`** finite-skeleton gate + **`_has_explicit_nominal_blocker`** extensions (**tanween**, **ال**, L5 **demonstrative**/**mabni**/**pronoun**). **L8B** derived-active **R2 kasra/damma** exclusion **reverted** — it caused **`verify_erqa_integrity` FAIL** (**8** rows); broad **`_has_strong_finite_verb_surface`** restored. **`--verify-erqa`:** **PASS** **corrupted_rows 0**. Benchmarks **`gold_csv`**: **494** **strict 288** / **mismatch 139** / **partial 66**; **4000** **strict 1936** / **mismatch 1442** / **partial 597**; **`mubtada`∧«فعل»** **`mismatch`**: **26** (**4000**). **Optional later:** smaller L8B fix for **فَعِيل/فَعُول** that preserves ERQA (risk: low) | `l14_jamid_mushtaq.py`, `l8b_verb_bab_governance.py`, `test_patch20_nominal_opening_collapse.py`, `test_stage14_jamid_mushtaq.py` |
| 2026-03-21 | **Patch 19 (Quran gold tooling):** **`choose_best_ayah_batch_result_after_repairs`** — repair attempts no longer overwrite **better** alignment counts; **`--verify-erqa`** fixed to **gold_csv** ayah reconstruction; **10000**-row **`alignment_coverage` → 1.0** (risk: low) | `ayah_batch_runner`, `run_quran_i3rab_comparison`, `test_patch19_repair_alignment.py` |
| 2026-03-24 | **Quran gold Partition A:** **`--verify-erqa`** CLI + **`verify_erqa_integrity`** — exact ERQA ayah-set regression gate (not contiguous mushaf prefix); batch summary **`newly_added_rows_this_patch`** / **`newly_added_ayahs_this_patch`** (risk: low) | `run_quran_i3rab_comparison`, `ayah_batch_runner`, `test_verify_erqa_integrity.py` |
| 2026-03-24 | **Master Execution Patches 16–18 (code):** **16** Stage15 **mafool_bih/فاعل** attachment; **17** L17 **IDAFA > OBJ** + **G007** gated on winning Stage15 relation; **18** comparator **`case_bucket_mismatch`** (majurūr-before-mabnī inference; **mudaf_ilaih** / **jussive** / **darf** bridges; **ظرف**→**noun** in `_l17_infer_family`). **Benchmarks:** **494** strict **267** / mismatch **131**; **4000** strict **1897** / mismatch **1378**; **10000** strict **4666** / mismatch **3637**, **alignment 0.9996** (risk: low) | `p16_18_exec/*.json`, `test_exec_patch16–18_*` |
| 2026-03-23 | **PIPELINE §7.5:** **Patches 16–18** diagnosis-first plan — **4000**-row structured-debug **`mismatch`** split (**`no_match` ~1191/1418**); recommend **16–17** upstream/attachment (**`mafool_bih`∧«فاعل»**, **`mudaf_ilaih`∧«مفعول به»**, nominal∧«فعل»); **18** comparator subset or hygiene; **dual baseline** (**272/143** vs **261/137**) documented; **A+B+C** after **18** (risk: none) | PIPELINE_MASTER_MEMORY, evaluation |
| 2026-03-23 | **Master Execution Patches 13–15 (comparator):** **(13)** gold **particle**/**darf** vs L17 **noun** «ظرف»; **(14)** **B32** **لنا** / **عليكم** / **عليهم** / **مما**; **(15)** **particle**/**fael** vs L17 **مضاف** (**2:43**). **Benchmarks:** **494** strict **261** / mismatch **137**; **2000** (~**1978** rows) strict **954** / mismatch **677**; **4000** (~**3976** rows) strict **1855** / mismatch **1418** (**+70** strict vs pre–**13–15** **C** snapshot **1785** / **1488**). Gate **15** checkpoint: **`PIPELINE_MASTER_MEMORY.md` §7.2.1** (risk: low) | comparator, quran_gold, PIPELINE_MASTER_MEMORY |
| 2026-03-23 | **PIPELINE §7.4:** Quran gold **`--max-rows 494`** vs **`2000`** benchmark roles (regression vs primary progress); next user **Patch 10** track = structured-debug diagnosis on **`family_conflict_particle`** in **2000**-row window before comparator edits (risk: none) | PIPELINE_MASTER_MEMORY |
| 2026-03-23 | **Master Execution Patches 10–12 (L17):** **B40** nominal **خبر** repair after **مبتدأ**/pointer + **لا رَيْبَ** handling; **B41** Quranic **ظرف** surface templates (**إذا**/**إذ**, …) including **`وإذ`** core fix + **فاعل** overwrite allowlist without **G007/G010** hard block; **B42** fused **لل/بال** → **اسم مجرور**. **494**-row **strict 256→272**, **mismatch 158→143**, **`darf` `no_match` →0** on window; tests `test_exec_patch10_11_12_l17.py` (risk: low) | L17_RULE_BASED_I3RAB,quran_gold |
| 2026-03-23 | **PIPELINE §7.1–7.3:** classified **master execution** ledger (**engine** vs **comparator/tier hygiene** vs **skipped**); post–**Patch 9** **494** baseline + **`mismatch`** taxonomy; micro-patch **ceiling 20** + **review gates** (**10** / **15** / **pre-20**); **9b** = **`gold_parser_limit`** hygiene **not** engine gain (risk: none) | PIPELINE_MASTER_MEMORY |
| 2026-03-23 | **Master Execution Patch 9:** **(9a)** L17 **B39** Stage15 **OBJ** → **مفعول به** repair; **(9b)** comparator **`gold_parser_limit`** partial for empty gold role (**tier hygiene**); **494**-row **mismatch 216→158**, **partial 23→80**, **mafool_bih** mismatch **26→25** (risk: low) | L17_RULE_BASED_I3RAB, comparator, quran_gold |
| 2026-03-23 | **Master Execution Patch 8:** **skipped** — **2:17** dual-محل / tense-display rows; no comparator normalization (risk: none) | comparator |
| 2026-03-23 | **Master Execution Patch 7:** comparator **particle**/**fael** vs L17 finite **فعل** — family gate + **fael** code + **مبني** bridge; **`family_conflict_particle` 20→5** on **494**-window (risk: low) | comparator, quran_gold |
| 2026-03-23 | **Master Execution Patch 6:** gold parser **leftmost** role match among patterns + comparator **`fael_mudari_masdar_an_nasb_ok`** (**مؤول** + **`( أَن`/`أن (`**); **494**-row **strict 245→255**, **mismatch 226→216**, **partial 23**, **alignment 1.0** (risk: low) | gold_prose_parser, comparator, quran_gold |
| 2026-03-23 | **Master Execution Patch 5 (comparator):** **sila_mawsul**/**verb** + L17 finite **فعل** → **`sila_mawsul`** code bridge; **494**-row **strict 229→245**, **mismatch 242→226** (risk: low) | comparator, quran_gold |
| 2026-03-23 | **Master Execution Patch 4 (comparator):** gold **fael**/**naib_fael**/**verb** vs L17 «فاعل»/«نائب فاعل» — narrow **`family_conflict_verb_vs_nonverb`** bypass; **494**-row **strict 226→229**, **mismatch 245→242** (risk: low) | comparator, quran_gold |
| 2026-03-23 | **Master Execution Patch 3 (L17):** **B33** `_apply_b33_fused_harf_jar_quran_surfaces` — **عَلَيْهِمْ** / **مِمَّا** when **L4** lacks operator row; **494**-row **strict 221→226**, **mismatch 250→245** (risk: low) | L17_RULE_BASED_I3RAB, quran_gold |
| 2026-03-23 | **Master Execution Patch 2 (comparator):** fused **لام/باء/إلى/min-mā** allowlisted surfaces — gold **harf_jar** vs L17 **noun/verb** PP-cluster display; **ism أنّ** spurious **harf_jar** excluded; **494**-row **strict 209→221**, **mismatch 262→250** (risk: low) | comparator, quran_gold |
| 2026-03-23 | **Master Execution Patch 1 (comparator):** **harf_jar** + **genitive** gold bucket (مجرور mention in same cell) vs L17 **built** حرف جر → strict bridge; **494**-row **strict 190→209**, **mismatch 281→262**, **alignment_coverage 1.0** (risk: low) | comparator, quran_gold |
| 2026-03-23 | **Batch 28.31:** Stage 15 **speech-frame containment** after reporting verbs — suppress matrix **SUBJ/OBJ/PRED** into quoted spans (يَا / vocative / finite / **هُوَ** quote), preserve **ال…** agent subjects; **is_qala_root_reporting_surface** + **is_reporting_speech_matrix_verb_surface** in L14 (risk: low) | DEPENDENCY_SYNTAX_BUILDER,L14 |
| 2026-03-21 | **Batch 28.30 — closed (validated):** reporting-verb finite recognition — `has_strong_true_verb_evidence` + **قُلْ**/**قُولُوا**/**نا** speech finite skeletons; **494**-row **182→190** strict, **287→281** mismatch, **pass_strict_ayahs 6**, **alignment_coverage 1.0**. **Next:** **L10B** clause-span — **إذا** not opened (risk: low) | L8B,L14,DEPENDENCY_SYNTAX_BUILDER |
| 2026-03-21 | **Roadmap (post‑28.30):** **priority** = **L10B** clause-span / locality-collapse reduction; **backlog** = **إذا وأخواتها** (explicit deferral until dedicated batch) — see **Section E.1** (risk: none) | roadmap |
| 2026-03-21 | Batch 28.29: comparator **particle**/**particle** **مبني** case bridge (extends **28.28**); **`gold_parser_limit`** partial tier for sparse gold role + low parser confidence + strong L17 — **not** strict; **494**-row strict **+2**, mismatch **−4**, **gold_parser_limit** rows **2** (risk: low) | comparator, quran_gold |
| 2026-03-23 | Batch 28.28: comparator **normalization** for gold **fael** on **verb** rows vs L17 «فعل» + **built** case inference; **494**-row strict **+22**, mismatch **−22**; resolved **no_match** **−22** (risk: low) | comparator, quran_gold |
| 2026-03-23 | Batch 28.27: evidence-based **re-ranking** of remaining blockers (structured-debug **494** / **2000**); **resolved-but-wrong** ≫ **unresolved**; next **Stage 15** attachment leverage **mafool_bih**∧**SUBJ** > **fael**∧**OBJ**; tooling `scripts/analyze_28_27_structured_debug.py` (risk: none) | scripts, evaluation |
| 2026-03-23 | Batch 28.26: Stage 15 extends 28.25 scan exclusion with **terminal و+ا** plural/imperative-plural verb shape (`_has_plural_imperative_verb_terminal_waw_alif_shape`); 494-row **mismatch** −1 vs 28.25; **fael**∧**OBJ** further ↓ on **2000**-row window (risk: low) | DEPENDENCY_SYNTAX_BUILDER |
| 2026-03-23 | Batch 28.25: Stage 15 Pass E2 / verbal-root scans exclude **finite-verb surfaces** (`has_strong_true_verb_evidence` + `_has_strong_finite_verb_surface`) from post-verbal nominal argument counting — reduces false **OBJ** on tokens where gold marks **fael** (e.g. mis-tagged قَالَ / شَاءَ); 494-row strict **+6** vs 28.24 baseline (risk: low) | DEPENDENCY_SYNTAX_BUILDER |
| 2026-03-23 | Batch 28.24: unified **clause locality** layer (`clause_locality`) so L17 in-clause scans align with Stage 15 **L10B** units when L16 is trivial main-only; conditional/multi-clause sentences still use L16 spans (risk: low) | clause_locality, builder, L17 |
| 2026-03-21 | Batch 28.23V production verification: Stage15 false **SUBJ** on gold **mafool_bih** drops on **2000**-row and **500**-ayah windows vs pre–28.23 builder; **mafool_bih** structured `no_match`↓ / `strict_structural_match`↑ on **~1978**-row window; **494**-row slice shows Stage15 movement without aggregate comparator shift (risk: low) | audit scripts only |
...
| 2026-03-21 | Quran iʿrāb gold comparison (Batch 28.3): strict comparator tiers, ayah-bounded safe writes, quarantine policy external to core engine (risk: low) | quran_gold tooling |

| 2026-03-22 | Quran gold Batch 28.15: ERQA accepted-row metadata canonicalization (`canonicalize_accepted_metadata`); display-aligned case/marker/signature; no engine stage/comparator change (risk: low) | quran_gold, accepted_row_serializer |

| 2026-03-22 | Quran gold Batch 28.16: L17 mudāriʿ marfūʿ vs false مبني على الفتح (ی/ت/ن-prefix and plural/damma disambiguation; low-conf passive override); Stage15 `Pass_B28_16_idafa_kasra_definite` for kasra-final + `ال` IDAFA; L17 مضاف head + NAIB_SUBJ repair after mudāriʿ; comparator unchanged (risk: low) | DEPENDENCY_SYNTAX_BUILDER, L17_RULE_BASED_I3RAB |
...
| 2026-03-21 | **Quran gold Batch 28.8:** L17 `_apply_b28_8_targeted_resolutions` — fused حرف جر surfaces, و/ف عطف, اسم موصول; pattern ranking from `quran_i3rab_discovery_rows.csv` blocked-L17 rows; strict comparator unchanged (risk: low) | L17_RULE_BASED_I3RAB,quran_gold |

| 2026-03-21 | **Quran gold Batch 28.9:** `ayah_unlock_ranker` — per-ayah near-pass classification, blocker CSVs, trapped-strict preview, `batch_28_9` JSON; evaluation/reporting only (risk: none) | quran_gold |

| 2026-03-21 | **Quran gold Batch 28.10:** L17 `_apply_b28_10_targeted_resolutions` — `لل…` fused jar, `والذ*`/`والتي*` mawsul after و; `B28_10_*` refs; `batch_28_10` summary + reporting CSVs; no comparator loosening (risk: low) | L17_RULE_BASED_I3RAB,quran_gold |

| 2026-03-21 | **Quran gold Batch 28.11:** IDAFA-before-PRED in L17 Stage15 read; مضاف إليه + بسم الله fallback; `ayah_completion_ranker` / `batch_28_11` artifacts; PASS_STRICT ayah count can increase without comparator change (risk: low) | L17_RULE_BASED_I3RAB,quran_gold |

| 2026-03-21 | **Quran gold Batch 28.12:** Accepted `erqa_i3rab.csv` rows serialize `system_i3rab` from accepted structured basis (`accepted_row_serializer`); legacy contradictory L11 prose preserved in `raw_system_i3rab_before_hardening`; comparator tiers unchanged (risk: none) | quran_gold |

| 2026-03-22 | **Quran gold Batch 28.13:** Modifier-aware accepted serialization — gold-resolved **نعت** (incl. ordinal hints) over generic «اسم مجرور» display; normalized `accepted_role` / signature coherence; tooling-only (risk: none) | quran_gold |

| 2026-03-22 | **Quran gold Batch 28.14:** L17 `B28_14_MUBTADA_PRED_HEAD` — sentence-initial **مبتدأ** from Stage15 nominal `PRED` head link; Fatiha 1:2 strict PASS_STRICT; comparator policy unchanged (risk: low) | L17_RULE_BASED_I3RAB,quran_gold |

| 2026-03-22 | Quran gold Batch 28.17: imperative + detached إيا alignment (L14/L15/L17); gold evaluation comparator unchanged (risk: low) | L14_JAMID_MUSHTAQ,DEPENDENCY_SYNTAX_BUILDER,L17_RULE_BASED_I3RAB,quran_gold |

| 2026-03-22 | Batch 28.18: Quran gold ayah unlock 1:5 and 2:1 via comparator+parser (huruf muqatta + waw-iyya structural bridge) (risk: low) | quran_gold,comparator,gold_prose_parser |

| 2026-03-22 | Batch 28.19 (comparator): core case_bucket_mismatch reduction on Quran gold pilot; `_infer_case_bucket_from_l17` (risk: low) | quran_gold,comparator |

| 2026-03-22 | Batch 28.19 (L17 nominal + Stage 15): `_apply_b28_19_nominal_short` — **مبتدأ/خبر** for **L10B** nominal, no verb, ≤1 Stage15 link, 1–3 nouns (`B28_19_NOMINAL_SHORT`); Stage 15 `Pass_B28_19_idafa_after_sifa_definite` after **SIFA** when kasra-final **B28_16** did not fire; tests `test_batch_28_19_nominal_short_l17.py` (risk: low) | DEPENDENCY_SYNTAX_BUILDER,L17_RULE_BASED_I3RAB |

| 2026-03-23 | Batch 28.20: unresolved **PARTICLE** + explicit **L4** `GEN`/حرف جر operator evidence -> resolved **حرف جر** only (`B28_20_HARF_JAR`); 494-row dry-run improved strict structural rows and reduced `no_match` without widening noun fallbacks (risk: low) | L17_RULE_BASED_I3RAB,quran_gold |

| 2026-03-23 | Batch 28.21: L17 `_apply_b28_21_mafool_bih_fallback` — narrow **مفعول به** recovery for **unresolved** accusative nouns after in-clause active verbs when Stage15 omits **OBJ**; `B28_21_MAFOOL_BIH_FALLBACK`; 494-row aggregate unchanged vs 28.20; comparator policy unchanged (risk: low) | L17_RULE_BASED_I3RAB,quran_gold |

| 2026-03-23 | Batch 28.22: L17 `_apply_b28_22_fael_fallback` — narrow **فاعل** recovery for **unresolved** marfūʿ nouns after in-clause finite **active** verbs when Stage15 omits **SUBJ** / intervening subject; `B28_22_FAEL_FALLBACK`; 494-row aggregate unchanged vs 28.21; comparator policy unchanged (risk: low) | L17_RULE_BASED_I3RAB,quran_gold |

| 2026-03-23 | Batch 28.23: Stage 15 **SUBJ→OBJ** correction for lone post-verbal **accusative** nouns (`Pass_B28_23_*`); reduces dominant false-**SUBJ** pattern on gold **mafool_bih** rows in global scan; comparator unchanged (risk: low) | DEPENDENCY_SYNTAX_BUILDER,quran_gold |

| 2026-03-24 | CV/cv_advanced unified on word-2-cv.py (pronunciation-aware article + shadda); tests test_compare_old_vs_new updated. (risk: low) | L6_PHONOLOGY,word-2-cv |

| 2026-03-25 | Next.js UI parity: `analyze_sentence.py` CLI runs `run_pipeline` and writes `--save-json`; duplicate `ui/src/app/api/analyze` route removed (risk: low) | analyze_sentence,UI,pipeline_orchestrator |
