...
export async function POST(req: NextRequest) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const step = (id: string, title: string, status: StepStatus) =>
        write("step_created", { id, title, status, ts: Date.now() });

      const stepUpdate = (id: string, status: StepStatus, detail?: string) =>
        write("step_updated", { id, status, detail, ts: Date.now() });

      // Steps (high-level only)
      step("s0", "تهيئة الطلب", "running");
      step("s1", "تشغيل التحليل المحلي (pipeline)", "queued");
      step("s2", "قراءة ملف JSON وتوليد ملخص", "queued");
      step("s3", "إخراج النتيجة النهائية", "queued");

      stepUpdate("s0", "done");
      stepUpdate("s1", "running");
      ...
      const forward = (streamName: "stdout" | "stderr", chunk: Buffer) => {
        const s = chunk.toString("utf-8");
        if (streamName === "stdout") stdoutChunks.push(s);
        write("log_delta", { stream: streamName, text: s });
      };

      child.stdout?.on("data", (c) => forward("stdout", c));
      child.stderr?.on("data", (c) => forward("stderr", c));
      ...
      child.on("close", async (code) => {
        ...
        if (code !== 0 && code !== null) {
          stepUpdate("s1", "error", `process exit code = ${code}`);
        ...
        try {
          ...
          stepUpdate("s2", "error", e?.message ?? String(e));
      ...
      child.on("error", (err) => {
        stepUpdate("s1", "error", err.message);
