...
## ✅ ملحق رسمي موحّد: فضاء السمات، الكلفة، المرشّحات + البرهانين

### 1) التعريف الرسمي لفضاء السمات $\mathcal{F}$

نعرّف فضاء السمات بوصفه اتحاد فضاء الصوامت وفضاء الصوائت:
$$
\mathcal{F} := \mathcal{F}_C \cup \mathcal{F}_V
$$

**فضاء الصوامت**:
$$
\mathcal{F}_C := \mathcal{P}\times\mathcal{M}\times\mathcal{Voi}\times\mathcal{Emph}\times\mathcal{Son}\times\cdots
$$
حيث:
- $\mathcal{P}$: مكان المخرج (شفوي/أسناني/لثوي/حنكي/لهوي/حلقي/حنجري…)
- $\mathcal{M}$: طريقة النطق (انفجاري/احتكاكي/مركب/أنفي/تقريبي…)
- $\mathcal{Voi}\in\{0,1\}$: جهر/همس
- $\mathcal{Emph}\in\{0,1\}$: استعلاء/تفخيم
- $\mathcal{Son}\in\{0,1\}$: صونورية

**فضاء الصوائت/الحركات**:
$$
\mathcal{F}_V := \mathcal{Height}\times\mathcal{Back}\times\mathcal{Round}\times\mathcal{Length}\times\cdots
$$
حيث:
- $\mathcal{Height}$: ارتفاع (low/mid/high)
- $\mathcal{Back}$: أمامية/خلفية
- $\mathcal{Round}\in\{0,1\}$: تدوير
- $\mathcal{Length}\in\{short,long\}$: طول

**مسافة السمات** (شبه معيار):
$$
d(f,g)=\sum_k w_k\,\delta_k(f_k,g_k),\quad w_k>0
$$
حيث $\delta_k$ فرق السمة $k$ (0 عند التطابق، موجب خلاف ذلك).

---

### 2) مولِّد المرشحين $G$ (منتهٍ)

مدخل التوليد $x$ يحتوي قالب خانات بطول منتهٍ $L$:
$$
	ext{Slots}(x)=\{s_1,\dots,s_L\}
$$
والخانـة $s_i$ نوعها $C$ أو $V$ أو ملحق/أداة.

نعرف:
$$
G(x)=\prod_{i=1}^L D(s_i)
$$
حيث $D(s_i)$ مجموعة منتهية من الخيارات المناسبة لنوع الخانة. إذن:
$$
|G(x)|=\prod_{i=1}^L |D(s_i)|<\infty
$$

---

### 3) دالة الكلفة $E$ (تضم القيود الصلبة كـ $\infty$)

$$
E(x,y)=H(x,y)+\sum_{j=1}^{m}\lambda_j\,\Phi_j(x,y),\quad \lambda_j\ge 0
$$
والمرشح $y\in G(x)$.

**البوابات الصلبة داخل $H$**:
$$
H(x,y)=\begin{cases}
0 & \text{إذا اجتاز } y \text{ جميع البوابات الصلبة}\\
\infty & \text{خلاف ذلك}
\end{cases}
$$
أمثلة بوابات صلبة: Gate-CV، Gate-Chain، Gate-Sig/Anchor، Gate-I‘rab/Bina’، Gate-OCP-hard.

**الدالة الجامعة**:
$$
F(x)=\arg\min_{y\in G(x)}E(x,y)
$$

---

### 4) برهان الانتهاء (Termination)

**الادعاء:** حساب $F(x)$ ينتهي.

**البرهان المبدئي:**
1. $|G(x)|<\infty$ لأن $L$ منتهٍ وكل $D(s_i)$ منتهٍ.
2. لكل $y\in G(x)$، $E(x,y)\in\mathbb{R}_{\ge 0}\cup\{\infty\}$.
3. فحص جميع العناصر في مجموعة منتهية واختيار الأصغر ينتهي بعد $|G(x)|$ خطوة.
4. إذن حساب $F(x)$ ينتهي.

---

### 5) برهان السلامة (Soundness)

**الادعاء:** ناتج $F(x)$ لا يخرق القيود الصلبة.

**البرهان المبدئي:**
1. افترض أن $y^*=F(x)$ يخرق بوابة صلبة؛ إذن $H(x,y^*)=\infty$ و$E(x,y^*)=\infty$.
2. إن وُجد مرشح صالح $y$ يمرّ بالبوابات، فـ $H(x,y)=0$ و$E(x,y)<\infty$.
3. يستحيل أن يكون $\infty$ هو الحد الأدنى؛ تناقض.
4. إذن $F(x)$ سليم بشرط وجود مرشح صالح واحد على الأقل. وإلا تُعاد حالة Unsat.

---

### 6) ضبط المخرج/الصفة + الاقتصاد الصوتي (Soft)

**كلفة الاقتصاد الصوتي**:
$$
\Phi_{\text{econ}}(y)=\sum_t \text{Effort}(segment_t)
$$

**كلفة الانتقال بين المقاطع/الفونيمات**:
$$
\Phi_{\text{trans}}(y)=\sum_i d(f_i,f_{i+1})
$$
حيث يعتمد $d$ على سمات المخرج/الصفة في $\mathcal{F}_C$ وسمات الحركة في $\mathcal{F}_V$.

---

### 7) مبرهنة “المقطع الأوسط” (Gate-Chain)

**التعريف:** المقطع $\sigma_i$ أوسط إذا $1<i<n$ في السلسلة $\sigma_1,\dots,\sigma_n$.

**شرط السلسلة الصلب:**
$$
\forall i\in\{2,\dots,n-1\}:\;\text{Link}(\sigma_{i-1},\sigma_i)=1\;\wedge\;\text{Link}(\sigma_i,\sigma_{i+1})=1
$$

**النتيجة:** أي مرشح يحوي مقطعًا يتيمًا يُرفض لأن $H=\infty$؛ وبذلك “المقطع الأوسط” مضمون داخل الدالة نفسها.

---

### 8) شرط “مسافة السمات” الذي يرجّح /u/ قرب المستعلي/المفخّم

**وزن سياقي للتفخيم**:
$$
\alpha(C)=1+\rho\cdot\mathbf{1}[\text{Emph}(C)=1],\quad \rho>0
$$

**كلفة اختيار الحركة في الموضع $i$**:
$$
\Phi_{\text{vowel}}(i)=\alpha(C_{i-1})\,d(V_i,C_{i-1})+\alpha(C_{i+1})\,d(V_i,C_{i+1})
$$

إذا كانت $\text{Emph}(C_{i\pm1})=1$ فإن أثر سمتي $Back/Round$ يتضخم داخل $d$. وبما أن /u/ عادةً $[+back,+round]$ مقارنةً بـ /i/ ($[-back,-round]$)، نحصل على:
$$
\Phi_{\text{vowel}}(V=u) < \Phi_{\text{vowel}}(V=i)
$$
عند التفخيم، بشرط زيادة أوزان سمات الخلفية/التدوير في $d$ ضمن هذا السياق.

---

**ملاحظة تنفيذية:** يفضّل ترميز السمات في مصفوفات ثابتة، وحساب $d$ بكلفة $O(k)$ لكل زوج، مع حفظ مؤقت (memoization) للحالات المتكررة لضبط التعقيد الكلي أثناء استعراض $G(x)$.
