...
-------------------------------------------
- Reads:
  1) patterns_csv: contains column "الوزن" (or "pattern"/"wazn")
  2) pairs_csv: contains columns: "الوزن" (gold) and "مثال" (word)
- Produces:
  - results CSV (row-level)
  - analysis TXT (summary + diagnostics)
...
def main():
    # Defaults: same directory as script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    patterns_csv = os.path.join(script_dir, "awzan_enhanced-atwah.csv")
    pairs_csv = os.path.join(script_dir, "awzan_claude.csv")
    out_csv = os.path.join(script_dir, "awzan_match_results.csv")
    out_txt = os.path.join(script_dir, "awzan_match_analysis.txt")
